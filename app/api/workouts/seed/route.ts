  ...
  try {
    ...
    for (const [goalType, workouts] of Object.entries(defaultWorkouts)) {
      ...
      for (const workoutTemplate of workouts) {
        ...
        console.log(
          `✅ Created workout: ${workout.name} with ${workout.exercises.length} exercises`
        );
      }
    }

    console.log(
      `🎉 Successfully seeded ${createdWorkouts.length} default workouts!`
    );
    ...
    return NextResponse.json(
      {
        error: "Failed to seed default workouts",
