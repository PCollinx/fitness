...
import { isBuildTime, withDatabaseAccess } from "@/lib/build-utils";

export async function GET(request: Request) {
  // Default response for build time
  const buildTimeFallback = {
    connected: false,
    tablesExist: false,
    userCount: 0,
    message:
      "Build-time placeholder response - no database connection attempted",
    buildTime: true,
    ...
    try {
      ...
      if (
        ...
        databaseStatus.message =
          "Connected to database, but tables don't exist yet";
      } else {
        console.error("Error accessing User table:", tableError);
        databaseStatus.message =
          "Connected to database, but error accessing tables";
      }
    }
